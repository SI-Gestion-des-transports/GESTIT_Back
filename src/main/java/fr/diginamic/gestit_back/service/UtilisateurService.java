...
public class UtilisateurService {
    ...
    public Utilisateur creerUtilisateur(UtilisateurDto utilisateurDto) {
        if (utilisateurRepository.findByEmail(utilisateurDto.email()).isPresent()) {
            ...
            return this.utilisateurRepository.save(new Utilisateur(utilisateurDto.nom(), utilisateurDto.email(),
                    utilisateurDto.motDePasse(), utilisateurDto.roles(), utilisateurDto.dateNonValide()));
        }
    }
    ...
    public Utilisateur modifierUtilisateur(UtilisateurDto nouveauUtilisateur, Integer idUtilisateur) {
        ...
        // LocalDate date = LocalDate.parse(nouveauUtilisateur.dateNonValide(),
        // formatter);
    ...
    public void desactiverUtilisateur(Integer idUtilisateur) {
        ...
        // Si utulisateur est présent, prendre la date à partir de laquelle
        // l'utilisateur n'est plus active.
        if (utilisateurOptional.isPresent()) {
            Utilisateur utilisateur = utilisateurOptional.get();
            LocalDate dateNonValide = utilisateur.getDateNonValide();
            // Si la dateNonValide (la date à partir de laquelle l'utilisateur n'est plus
            // active) est présente, cherche les covoiturages organisés par cet utilisateur
            if (dateNonValide != null) {
                /*
                 * @OneToMany(mappedBy = "organisateur")
                 * private Set<Covoiturage> covoituragesOrganises = new HashSet<>();
                 * => l'entity Utilisateur n'a pas accès sur les données de covoiturages, la
                 * fonction : "utilisateur.getCovoituragesOrganises()" ne marche pas; il faut
                 * passer par la classe Covoiturage
                 * (covoiturageRepository.findCovoituragesByOrganisateur(utilisateur) )
                 */
                Set<Covoiturage> covoiturageOrganise = covoiturageRepository
                        .findCovoituragesByOrganisateur(utilisateur);
